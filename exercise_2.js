...
// Réflexion 2: en partant de l'Est pour aller à l'Ouest, les bâtiments auront une vue ensoleillée si et seulementla hauteur du bâtiment est plus grande que la hauteur maximale enregistrée. => heights[i] > maxHeight = count ++ & maxHeight = heights[i]

// Définir une fonction countBuildingsWithSweetThatSunnyView qui prend un tableau (heights) en entrée
function countBuildingsWithSweetThatSunnyView(heights) {
  ...
  for (var i = heights.length - 1; i > 0; i--) {
    ...
    if (heights[i] > maxHeight) {
      count++;          // Si c'est le cas, incrémenter le compteur car ce bâtiment a une vue ensoleillée
...
// Appeler la fonction countBuildingsWithSweetThatSunnyView avec le tableau de hauteurs
console.log(countBuildingsWithSweetThatSunnyView(buildingHeights)); // Doit afficher 4
